...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    return { title: "Macchina — Espressamente" };
  }
}

export default async function MacchinaDetailPage({ params }: PageProps) {
  const { slug } = await params;
  ...
  if (product.productType !== "MACCHINA" && product.productType !== "ACCESSORIO") notFound();
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
        ...
        <div>
          ...
          <div className="mt-10 pt-6 border-t border-brand-100 flex flex-col sm:flex-row gap-3">
            <Link
              href="/contatti"
              className="inline-flex items-center justify-center flex-1 px-6 py-3.5 bg-gradient-to-r from-brand-400 to-brand-500 hover:from-brand-500 hover:to-brand-600 text-white font-medium rounded-xl transition-all duration-300 hover:shadow-lg hover:shadow-brand-400/20 hover:scale-[1.01] active:scale-[0.99]"
            >
              Richiedi preventivo
            </Link>
            <Link
              href="/assistenza"
              className="inline-flex items-center justify-center flex-1 px-6 py-3.5 border border-brand-300 text-brand-900 font-medium rounded-xl hover:bg-brand-50 hover:border-brand-400 transition-all duration-200"
            >
              Assistenza
            </Link>
          </div>
